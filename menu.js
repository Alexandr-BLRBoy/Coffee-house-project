...
const getProducts = async () => {
    ...
    const result = await fetch("./Products.json").then((response) => response.json());
...
const createCardsItem = (category, products) => {
    const cardsWrapperBlock = document.querySelector('.coffee_container__wrapper');
            ...
            cardItem.addEventListener('click', () => {
                ...
                tabSize.forEach(item => {
                    item.addEventListener('click', () => {
                        ...
                        const price = parseFloat(elem.sizes[item.dataset.size]["add-price"]);
                ...
                tabAdditives.forEach(item => {
                    item.addEventListener('click', function() {
                        ...
                        let newTotal = currentTotal + selectedAdditives.reduce((acc, val) => acc + val, 0);
...
const handleTabClick = async () => {
    ...
    const createCardsItem = async (elem) => {
        ...
        const category = (event.currentTarget.querySelector('p').innerHTML.toLowerCase());
        ...
        filteredProducts.forEach((elem) => {
            ...
            const additivesNamesAfterClick = elem.additives.map(additive => additive.name)
